...
void MetaRead::read(int sfRow)
{
    if (isDebug || G::isLogger) G::log("MetaRead::read");
    if (isDebug)
    {
        qDebug().noquote() << "MetaRead::read"
                           << "start  row =" << sfRow
                           << "dm->sf->rowCount() =" << dm->sf->rowCount()
                              ;
    }
    ...
    if (sfRow >= dm->sf->rowCount()) {
        if (G::isWarningLogger)
        qWarning() << "WARNING MetaRead::read"
                   << "abort =" << abort
                   << "instance =" << instance << "dm->instance =" << dm->instance
                   << "dm->sf->rowCount() =" << dm->sf->rowCount()
                   << "row =" << sfRow << "FAILED RANGE CHECK"
    ...
    if (!sfIdx.isValid()) {
        if (G::isWarningLogger)
        qWarning().noquote() << "WARNING MetaRead::read  "
                             << "invalid sfidx =" << sfIdx
                                 ;
        return;
    }
    ...
    // read icon if in icon range
    if (sfRow >= firstIconRow && sfRow <= lastIconRow) {
        iconLoaded = dm->iconLoaded(sfRow, instance);
        if (!iconLoaded) {
            // get thumb
            bool isVideo = false;
            isVideo = dm->index(dmRow, G::VideoColumn).data().toBool();
            ...
            thumbLoaded = thumb->loadThumb(fPath, image, instance, "MetaRead::readIcon");
            if (ms > 20)
                qDebug() << "MetaRead::readIcon thumb->loadThumb time ="
                         << ms << "ms" << "row =" << dmRow
                            ;
            if (isVideo && !abort) {
                rowsWithIcon.append(dmRow);
    ...
    // Updates
    if (!metaLoaded && !iconLoaded) {
        emit addMetadataAndIconInDM(metadata->m, dmIdx, pm, instance, "MetaRead::read");
    }

    if (!metaLoaded && iconLoaded) {
        emit addToDatamodel2(metadata->m, "MetaRead::read");
    }

    if (metaLoaded && !iconLoaded) {
        emit setIcon(dmIdx, pm, instance, "MetaRead::read");
    }
...
*/
    ...
    while (a < sfRowCount || b >= 0) {
        if (startRow != -1) {
            qDebug() << "MetaRead::run startRow change =" << startRow;
            a = startRow;
            b = startRow - 1;
            sfRowCount = dm->sf->rowCount();
            lastRow = sfRowCount - 1;
...
//    emit setIcon(dmIdx, pm, instance, "MetaRead::readIcon");
//    if (ms > 20)
//        qDebug() << "MetaRead::readIcon emit setIcon time ="
//                 << ms << "ms" << "row =" << dmRow
//                    ;
//    rowsWithIcon.append(dmRow);
...
//void MetaRead::changeCurrentFile()
///*
//    Signal MW::fileSelectionChange for the current selection, which will also
//    trigger the ImageCache to rebuild.
//*/
