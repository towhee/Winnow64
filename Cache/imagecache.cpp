...
//    for (int id = 0; id < decoderCount; ++id) {
//        qDebug() << __PRETTY_FUNCTION__ << "decoder[id]->isRunning()" << id << decoder[id]->isRunning();
//    }

//    QString isRun;
//    if (isRunning()) isRun = "true";
//    else isRun = "false";
//    G::track(__PRETTY_FUNCTION__, "Start: isRunning = " + isRun);
...
//    if (isRunning()) isRun = "true";
//    else isRun = "false";
//    G::track(__PRETTY_FUNCTION__, "Done:  isRunning = " + isRun);
...
int ImageCache::getImCacheSize()
{
    ...
    if (debugCaching) {
        qDebug().noquote() << __PRETTY_FUNCTION__ << " cache size =" << cacheMB;
    }
    return cacheMB;
}
...
*/
    ...
    for (int i = 0; i < icd->cacheItemList.count(); i++) {
        if (icd->cacheItemList.at(i).fPath == currentPath) {
            icd->cache.key = i;
            if (debugCaching) {
                qDebug().noquote() << __PRETTY_FUNCTION__ << "key =" << i;
            }
            return;
        }
    }
    qWarning() << __PRETTY_FUNCTION__ << "FPATH NOT FOUND:" << currentPath;
}
...
//            if (isCached) emit updateCacheOnThumbs(fPath, false, icd->cache.targetFirst,icd->cache.targetLast);
    ...
    if (!checkForOrphans) {
        if (debugCaching) {
            qDebug().noquote() << __PRETTY_FUNCTION__ << "     No orphans found";
        }
    }
}
...
//        qWarning() << __PRETTY_FUNCTION__ << m.fPath << "not in cacheKeyHash";
        return;
    }

    if (row >= icd->cacheItemList.length()) {
        qWarning() << __PRETTY_FUNCTION__<< "row not in icd->cacheItemList";
        return;
    }
...
*/
    ...
    setCurrentPosition(currentPath, __PRETTY_FUNCTION__);
}
...
void ImageCache::setCurrentPosition(QString path, QString src)
{
    ...
    */
    ...
    if (debugCaching) {
        qDebug();
        qDebug().noquote() << __PRETTY_FUNCTION__ << path << "src =" << src;
    }
    mutex.lock();
    currentPath = path;
    mutex.unlock();
...
*/
    if (debugCaching) {
        ...
        qDebug().noquote() << __PRETTY_FUNCTION__ << "     decoder" << id << "row =" << k
                 << decoder[id]->fPath;
    }
