...
#include <functional>
#include <thread>
#include <QDebug>// (also safe to include here)
...
public:
  ...
  // Set callback to receive per-task progress updates (0..total).
  void set_progress_callback(std::function<void(int progress, int total)> callback);

  Worker* get_worker() const { return m_worker.get(); }
...
private:
  ...
  // progress callback
  std::function<void(int progress, int total)> m_progress_callback;
