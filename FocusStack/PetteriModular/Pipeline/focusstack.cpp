  ...
  m_wait_images(0.0f)
{
  ...
  qDebug() << "FocusStack::FocusStack  m_threads =" << m_threads;
  reset();
}
...
void FocusStack::set_progress_callback(std::function<void(int progress, int total)> callback)
{
...
void FocusStack::start()
{
  m_worker = std::make_unique<Worker>(m_threads, m_logger);
    ...
    // Attach progress callback if user provided one
    if (m_progress_callback)
        m_worker->progress_callback = m_progress_callback;
