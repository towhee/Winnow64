...
bool FSFusionPMax::fusePMax(const std::vector<cv::Mat>& grayImgs,
...
{
    const QString srcFun = "FSFusionPMax::fusePMax";
    if (G::FSLog) G::log(srcFun, "One-shot PMax via streaming API");

    const int N = (int)grayImgs.size();
    if (N <= 0 || (int)colorImgs.size() != N)
        return false;

    // Validate invariants expected by streaming
    const cv::Size alignSz = grayImgs[0].size();
    const int outDepth0 = colorImgs[0].depth();

    for (int s = 0; s < N; ++s)
    {
        if (FSFusion::isAbort(abortFlag)) return false;
        if (grayImgs[s].empty() || colorImgs[s].empty()) return false;
        if (grayImgs[s].type() != CV_8U) return false;               // streaming expects CV_8U gray
        if (grayImgs[s].size() != alignSz) return false;
        if (colorImgs[s].size() != alignSz) return false;
        if (colorImgs[s].depth() != outDepth0) return false;
    }

    // Reset engine streaming state
    reset();

    // Local "owner" holds geometry/crop rules for the engine
    FSFusion owner;

    // In one-shot, treat inputs as already in ALIGN space and also as ORIG space:
    // - no loader padding to undo
    // - so validAreaAlign is full-frame and origSize == alignSize
    owner.alignSize = alignSz;
    owner.origSize  = alignSz;
    owner.validAreaAlign = cv::Rect(0, 0, alignSz.width, alignSz.height);
    owner.outDepth  = outDepth0; // informational

    // Pass-1: stream slices
    for (int s = 0; s < N; ++s)
    {
        if (!streamSlice(s,
                         grayImgs[s],
                         ...
                         progressCb))
        {
            qWarning().noquote() << "WARNING:" << srcFun << "streamSlice failed at" << s;
            return false;
        }
    }

    // streamFinish signature still carries these; for PMax they are not used.
    QStringList dummyPaths;
    std::vector<Result> dummyGlobals;
    dummyGlobals.resize(N);

    // Pass-2: finalize
    if (!streamFinish(outputColor,
                      opt,
                      depthIndex16,
                      dummyPaths,
                      dummyGlobals,
                      ...
                      progressCb))
    {
        qWarning().noquote() << "WARNING:" << srcFun << "streamFinish failed.";
        return false;
    }
...
// ============================================================
// Streaming PMax (moved from your old streamPMaxSlice/Finish())
// ============================================================
