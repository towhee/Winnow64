...
/*
    1.	Thread Safety: The use of std::shared_mutex ensures that multiple threads can read
        simultaneously while writes are exclusive.
        •	Methods like count(), getKeys(), contains(), and find() use std::shared_lock for
            shared access.
        •	Methods that modify the data (insert(), remove(), clear()) use std::unique_lock
            to ensure exclusive access.
    2.	Traversal Safety: In the insert and remove functions, we carefully traverse the linked
        list and ensure proper locking when modifying nodes.
    3.	No Deadlocks: Each function locks only once and for the duration necessary, avoiding
        potential deadlocks.
*/
...
public:
    ...
    {
        std::shared_lock<std::shared_mutex> lock(mutex_);
        // qDebug() << "HashNode::contains start" << key;
        containsInUse = true;
        auto node = head.get();
        const QString keyString = key;
        while (node != nullptr)
        {
            // Check if the node's key is valid
            // What type is nodeKey??? not a QString???
            const auto nodeKey = node->getKey();
            // if (!nodeKey.isNull() && !nodeKey.isEmpty()) {
                // Perform comparison
                if (nodeKey == keyString)
                {
                    return true;
                }
            // }
        }
        // qDebug() << "HashNode::contains end" << key;
        containsInUse = false;
        return false;
    }
    ...
    {
        std::unique_lock<std::shared_mutex> lock(mutex_);
        // if (containsInUse) qDebug() << "HashNode::insert containsInUse";

        auto node = head.get();
        HashNode<K, V>* prev = nullptr;

        // Traverse to find if the key already exists
        while (node != nullptr && node->getKey() != key)
        {
            prev = node;
            node = node->next.get();
        ...
        if (node == nullptr)
        {
            ...
            if (prev == nullptr)
            {
                head = std::move(newNode);  // Insert at head
            }
            else
            {
                prev->next = std::move(newNode);  // Insert at next position
            }
        }
        else
        {
    ...
    {
        std::unique_lock<std::shared_mutex> lock(mutex_);
        // qDebug() << "HashNode::remove" << key;
        // if (containsInUse) qDebug() << "HashNode::remove containsInUse";
        HashNode<K, V>* prev = nullptr;
        auto node = head.get();
        ...
        if (prev == nullptr)
        {
            head = std::move(node->next);  // Move head to next node
        }
        else
        {
            prev->next = std::move(node->next);  // Bypass the node
        }
    }
...
private:
    std::unique_ptr<HashNode<K, V>> head;
    mutable std::shared_mutex mutex_;  // Ensures thread safety
    mutable bool containsInUse;
};
