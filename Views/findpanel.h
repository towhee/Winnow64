...
/*
    ...
    See notes/Documentation.txt "The Find Dock (Here and Everywhere)".
    ...
    THE FACET TREE IS THE Filters WIDGET ITSELF, not a copy of it. Both scopes render the
    same categories, the same checkboxes, the same include/exclude gestures, the same
    ...
    HERE narrows what is loaded: checking an item re-runs the proxy filter and the counts
    move. EVERYWHERE searches the index and its result is not a filter but a set of images
    to LOAD or ADD -- so the footer changes verb with the scope rather than the panel
    changing shape.

    SWITCHING SCOPE CARRIES THE SEARCH TEXT, AND CLEARS THE CHECKED FACETS. The text is
    the question, and carrying it is the hand-off in both directions: a search here that
    ...
    The facet CHECKS deliberately do not carry, because the two scopes are different
    ...
    THE SEARCH BOX IS SHARED. F2 focuses it in Here, Shift+F2 focuses it in Everywhere, so
    the pairing those two shortcuts always implied is now literally true. The text is
    ...
    of the grid Winnow already has, so Everywhere reports how many images matched and
...
public:
    ...
    /* Re-read the catalog size and, in Everywhere scope, its facets. Called when the dock
       becomes visible and after a folder load has added to the catalog. */
    void refresh();
...
signals:
    ...
    /* Back in Here scope: the tree is holding the CATALOG's values and must be rebuilt
    ...
    void rebuildHereFacetsRequested();

private slots:
    /* Run the Everywhere query. Debounced; a no-op in Here scope, where the tree drives
...
private:
    void updateFooter();
    void applyScope();
    /* The query the box and the checked facets currently describe. */
    CatalogQuery currentQuery() const;

    Filters *filters = nullptr;

    QToolButton *hereBtn = nullptr;
    QToolButton *everywhereBtn = nullptr;
    ...
    /* The most recent Everywhere result, held so Load does not have to re-run the query
       -- and so what gets loaded is exactly what the count described. */
    QStringList results;
    int totalMatches = 0;

    Scope currentScope = Here;
    bool scanning = false;
