    ...
    : QTreeView(parent)
{
    ...
    connect(this, &InfoView::setValDm, dm, &DataModel::setValDm);
...
{
/*
    Updates the datamodel for info items that can be edited: title, creator, copyright,
    email and url (at present). This is used in Embel (ie image title) and "Show shooting
    data".

    dataChanged is triggered twice for the same edit so count is used to only process
    once.

    The signal dataEdited is emitted, which triggers Embel to update the text fields.
    This will only work if Embel::isRemote == false.

    The signal updateFilter is emitted, which triggers BuildFilters to rebuild category
...
*/
    ...
    if (count == 0) {
        ...
        QString field;
        // dm->sf->suspend(true,  "InfoView::dataChanged count = 0");
        if (isEditable) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (field == "Title*") {
                    ...
                    emit setValDm(dmRow, G::TitleColumn, s, dm->instance, src, Qt::ToolTipRole);
                }
                if (field == "Creator*") {
                    QString s = idx1.data().toString();
                    emit setValDm(dmRow, G::CreatorColumn, s, dm->instance, src, Qt::EditRole);
                    emit setValDm(dmRow, G::CreatorColumn, s, dm->instance, src, Qt::ToolTipRole);
                }
                if (field == "Copyright*") {
                    QString s = idx1.data().toString();
                    emit setValDm(dmRow, G::CopyrightColumn, s, dm->instance, src, Qt::EditRole);
                    emit setValDm(dmRow, G::CopyrightColumn, s, dm->instance, src, Qt::ToolTipRole);
                }
                if (field == "Email*") {
                    QString s = idx1.data().toString();
                    emit setValDm(dmRow, G::EmailColumn, s, dm->instance, src, Qt::EditRole);
                    emit setValDm(dmRow, G::EmailColumn, s, dm->instance, src, Qt::ToolTipRole);
                }
                if (field == "Url*") {
                    QString s = idx1.data().toString();
                    emit setValDm(dmRow, G::UrlColumn, s, dm->instance, src, Qt::EditRole);
                    emit setValDm(dmRow, G::UrlColumn, s, dm->instance, src, Qt::ToolTipRole);
                }
...
void InfoView::setColumn0Width()
{
    ...
    QFont ft = this->font();
    ft.setPixelSize(static_cast<int>(G::strFontSize.toInt() * 1.333/* * G::ptToPx*/));
    QFontMetrics fm(ft);
    #ifdef Q_OS_WIN
