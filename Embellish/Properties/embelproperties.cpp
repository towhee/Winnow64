...
{
    if (G::isLogger) G::log("EmbelProperties::EmbelProperties");
...
{
    if (G::isLogger) G::log("EmbelProperties::initialize");
...
{
    if (G::isLogger) G::log("EmbelProperties::effectContextMenu");
...
{
    if (G::isLogger) G::log("EmbelProperties::newEffect");
    QString effect = (static_cast<QAction*>(sender()))->text();
    qDebug() << "EmbelProperties::newEffect" << "effect =" << effect;
    ...
    e->build("", "EmbelProperties::newEffect");
}
...
Update to:              1, 2, 3
*/
    if (G::isLogger) G::log("EmbelProperties::updateEffectOrderAfterDeletion");
...
Update to:              1, 2, 3
*/
    if (G::isLogger) G::log("EmbelProperties::updateBorderOrderAfterDeletion");
    // Get a standardItem as it supports sorting of the children, and the borders may be sorted
...
*/
    if (G::isLogger) G::log("EmbelProperties::updateBorderLists");
    borderList.clear();
...
/*
    ...
    vectors.
*/
    if (G::isLogger) G::log("EmbelProperties::updateAnchorObjects");
...
{
    if (G::isLogger) G::log("EmbelProperties::diagnosticStyles");
...
{
    if (G::isLogger) G::log("EmbelProperties::diagnosticVectors");
    rpt << "\n" << "\nVectors:";
    if (b.length()) rpt << "\n" << "Border(s)";
...
{
    if (G::isLogger) G::log("EmbelProperties::diagnosticModel");
...
{
    if (G::isLogger) G::log("EmbelProperties::renameSettingKey");
    // if name is blank then could remove all child items in settings
    if (oldName == "") {
        qDebug() << "EmbelProperties::renameSettingKey" << "Error: Blank name";
        return;
    }
...
{
    if (G::isLogger) G::log("EmbelProperties::renameCurrentStyle");
//    BarBtn *btn = qobject_cast<BarBtn*>(sender());
...
{
    if (G::isLogger) G::log("EmbelProperties::renameCurrentTemplate");
...
*/
    if (G::isLogger) G::log("EmbelProperties::setCurrentTemplate", name);
    if (name == "") return;
    templateName = name;
...
*/
    if (G::isLogger) G::log("EmbelProperties::sortTemplateList");
...
*/
    ...
    syncWinnets("EmbelProperties::readTemplateList");
}

QString EmbelProperties::uniqueTemplateName(QString name)
{
    if (G::isLogger) G::log("EmbelProperties::uniqueTemplateName");
    setting->beginGroup("Embel/Templates");
...
{
    if (G::isLogger) G::log("EmbelProperties::uniqueTileName");
    setting->beginGroup("Embel/Tiles");
    QStringList keys = setting->childKeys();
    setting->endGroup();
    qDebug() << "EmbelProperties::uniqueTileName" << name << keys;
    bool nameExists = true;
    QString newName = name;
    int count = 0;
    while (nameExists) {
...
{
    if (G::isLogger) G::log("EmbelProperties::uniqueGraphicName");
...
{
    if (G::isLogger) G::log("EmbelProperties::uniqueTokenName");
...
{
    if (G::isLogger) G::log("EmbelProperties::copyStyle");
    ...
    qDebug() << "EmbelProperties::copyStyle" << toTemplate << copyName;
...
{
    if (G::isLogger) G::log("EmbelProperties::copyTemplate");
...
*/
    if (G::isLogger) G::log("EmbelProperties::saveTemplateToFile");
...
*/
    if (G::isLogger) G::log("EmbelProperties::readTemplateFromFile");
...
//        qDebug() << "EmbelProperties::readTemplateFromFile" << a << b;
...
{
    if (G::isLogger) G::log("EmbelProperties::readTileList");
    tileList.clear();
...
{
    if (G::isLogger) G::log("EmbelProperties::readGraphicsList");
    graphicList.clear();
    setting->beginGroup("Embel/Graphics");
...
{
    if (G::isLogger) G::log("EmbelProperties::manageTiles");
...
{
    if (G::isLogger) G::log("EmbelProperties::updateTileList");
    tileList.clear();
    setting->beginGroup("Embel/Tiles");
    tileList << setting->allKeys();
    setting->endGroup();
...
{
    if (G::isLogger) G::log("EmbelProperties::manageGraphics");
    ...
    dlg.exec();
    updateGraphicList();
    qDebug() << "EmbelProperties::manageGraphics" << graphicList;
}
...
void EmbelProperties::updateGraphicList(QString oldName, QString newName)
{
    ...
    setting->endGroup();
    graphicList.sort();
    qDebug() << "EmbelProperties::updateGraphicList" << graphicList;
...
{
    if (G::isLogger) G::log("EmbelProperties::readMetadataTemplateList");
...
*/
    if (G::isLogger) G::log("EmbelProperties::updateMetadataTemplateList");
...
{
    if (G::isLogger) G::log("EmbelProperties::newTemplate");
    templateName = Utilities::inputText("New Template",
                                        "Enter new template name",
                                        templateList);
    qDebug() << "EmbelProperties::newTemplate" << templateName;
...
//    // set all templates except templateName isCurrent = false
    ...
    templateListEditor->setValue(templateName);
    syncWinnets("EmbelProperties::newTemplate");
}
...
void EmbelProperties::newTemplateFromImport(QString name)
{
    ...
    qDebug() << "EmbelProperties::newTemplateFromImport" << "templateListEditor->setValue(templateName)" << name;
    templateListEditor->setValue(name);
    emit syncEmbellishMenu();
    syncWinnets("EmbelProperties::newTemplateFromImport");
}

void EmbelProperties::syncWinnets(QString src)
{
/*
    This function runs every time Winnow is opened, when the Embellish Dock is
    created. It keeps winnets up-to-data:

        - checks necessay libraries are present
            - win Qt5Core.dll
            - mac QtCore.framemwork
        - checks there is a winnet for every embellish template
        - removes old winnets

    Winnets are small executables that act like photoshop droplets. The original is
    located in the same folder as the Winnow executable/app. A copy is made for each
    Embellish template in the Winnets folder:

        Windows: user/AppData/Roaming/Winnow/Winnets
        Mac:: user/Library/Application Support/Winnow/Winnets

    Winnets send a list of files and a template name to Winnow to be embellished.
    For example, in order for Winnow to embellish a series of files that have been
    exported from lightroom, Winnow needs to know which embellish template to use.
    Instead of sending the files directly to Winnow, thay are sent to an
    intermediary program (a Winnet) that is named after the template. The Winnet (ie
    EmbellishZen2048) receives the list of files, inserts the strings Embellish" and
    ...
    arg[1+] = path to each image being exported to be embellished

    Only arg[1] is used to determine the directory to open in Winnow.

    The main Winnet is called winnet.exe (Win) or winnet (Mac) and it is copied and
    renamed to the template name when a new template is created. The Winnets need to
    be synced when templates are created, renamed or deleted.
...
*/
    ...
    QString appDataWinnetsPath = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation);
    // Winnets subfolder
    appDataWinnetsPath += "/Winnets";
    ...
    // list of all executables in the Winnow/Winnets folder
    dir.setPath(appDataWinnetsPath);
    dir.setFilter(QDir::Files | QDir::NoDotAndDotDot);
    QStringList existingExecutables;
    // all files in appDataWinnetsPath, filter this separately in WIN/MAC
    ...
    // strip ".exe" or ".app" to compare to adjTemplateList
    existingExecutables.replaceInStrings(ext, "");

    // Convert templateList into a adjTemplateList ("Embellish" + template name)
    QStringList adjTemplateList;
    for (const QString &templateName : templateList) {
        if (templateName == "Do not Embellish") continue;
        adjTemplateList << "Embellish" + templateName;
    }

    // add if missing item in templateList
    // QFile Winnet(winnetPath);
    // qDebug() << "Winnet.exists() =" << Winnet.exists();
    for (const QString &name : adjTemplateList) {
        if (!existingExecutables.contains(name)) {
    ...
    for (int i = existingExecutables.length() - 1; i >= 0; i--) {
        if (!adjTemplateList.contains(existingExecutables.at(i))) {
            QString name = existingExecutables.at(i);
            // ignore non "Embellish*" files
...
{
    if (G::isLogger) G::log("EmbelProperties::invokeFromAction");
...
{
    if (G::isLogger) G::log("EmbelProperties::doNotEmbellish");
...
*/
    if (G::isLogger) G::log("EmbelProperties::swapBorders");
    ...
    e->build("", "EmbelProperties::swapBorders");
}
...
*/
    if (G::isLogger) G::log("EmbelProperties::moveBorderUp");
    BarBtn *btn = qobject_cast<BarBtn*>(sender());
//    QModelIndex idx = btn->index;
...
{
    if (G::isLogger) G::log("EmbelProperties::moveBorderDown");
    BarBtn *btn = qobject_cast<BarBtn*>(sender());
//    QModelIndex idx = btn->index;
    QModelIndex idx = getItemIndex(btn->itemIndex);
    if (!idx.isValid()) return;
...
/*
    ...
    being sorted.
*/
    if (G::isLogger) G::log("EmbelProperties::swapEffects");
    ...
    /*
    qDebug() << "EmbelProperties::swapEffects"
             << "item->data() =" << item->data()
...
//                */
    ...
    for (int i = 0; i < model->rowCount(par); ++i) {
        ...
        qDebug() << "EmbelProperties::swapEffects" << "Before swap"
                 << "row =" << i
...
//    /*
    for (int i = 0; i < model->rowCount(par); ++i) {
        QModelIndex idx = model->index(i, 0, par);
        QString name = idx.data().toString();
        qDebug() << "EmbelProperties::swapEffects" << "After  swap"
                 << "row =" << i
                 << "name =" << name.leftJustified(8)
                 << "order =" << idx.data(UR_SortOrder).toInt()
                    ;
...
//                */
    ...
    // refresh the graphicsScene
    e->build("", "EmbelProperties::swapEffects");
...
*/
    if (G::isLogger) G::log("EmbelProperties::moveEffectUp");
    BarBtn *btn = qobject_cast<BarBtn*>(sender());
    /*
    QWindow *wBtn = new QWindow;
    btn->createWindowContainer(wBtn);
    if (wBtn == nullptr) qDebug() << "NULL";
    qDebug() << "EmbelProperties::moveEffectUp";
...
//    QModelIndex idx = btn->index;
    if (!idx.isValid()) {
        qDebug() << "EmbelProperties::moveEffectUp" << "idx.isValid() =" << idx.isValid();
        return;
    }
...
//                */
    ...
    qDebug() << "EmbelProperties::moveEffectUp" << "row =" << row;
    if (row == 0) return;
...
{
    if (G::isLogger) G::log("EmbelProperties::moveEffectDown");
    BarBtn *btn = qobject_cast<BarBtn*>(sender());
    QModelIndex idx = getItemIndex(btn->itemIndex);
//    QModelIndex idx = btn->index;
    if (!idx.isValid()) return;
    ...
    for (row = 0; row < styleItem->rowCount(); ++row) {
        QString sortedItemName = styleItem->child(row)->data(Qt::DisplayRole).toString();
//        /*
        qDebug() << "EmbelProperties::moveEffectDown"
                 << "row =" << row
                 << "effectName =" << effectName
...
{
    if (G::isLogger) G::log("EmbelProperties::syncBorderVector");
    for (int i = 0; i < model->rowCount(bordersIdx); ++i) {
        QModelIndex bIdx = model->index(i, 0, bordersIdx);
...
{
    if (G::isLogger) G::log("EmbelProperties::sortEffectList", "Style: " + style);
    if (!styleMap.contains(style)) {
        qDebug() << "EmbelProperties::sortEffectList" << style << "not in list";
        return false;
    }
...
*/
    if (G::isLogger) G::log("EmbelProperties::itemChange");
...
//    QModelIndex index = idx.data(UR_QModelIndex).toModelIndex();
    qDebug() << "EmbelProperties::itemChange"
             << " =" << templateId
...
{
    if (G::isLogger) G::log("EmbelProperties::itemChangeTemplate");
...
void EmbelProperties::itemChangeExport(QModelIndex idx, QVariant v, QString source)
{
    ...
    if (source == "saveMethod") {
        ...
        qDebug() << "EmbelProperties::itemChangeExport" << source << saveMethod;
        int row = idx.row();
...
{
    if (G::isLogger) G::log("EmbelProperties::itemChangeGeneral");
    QString path = templatePath + "General/" + source;
//    qDebug() << "EmbelProperties::itemChangeGeneral" << path;
    ...
    e->build("", "EmbelProperties::itemChangeGeneral");
}

void EmbelProperties::itemChangeBorder(QModelIndex idx)
{
    if (G::isLogger) G::log("EmbelProperties::itemChangeBorder");
    QVariant v = idx.data(Qt::EditRole);
    QString source = idx.data(UR_Source).toString();
    QString parent = idx.parent().data(UR_Name).toString();
...
//    diagnostics(idx);
    ...
    if (source == "topMargin") {
        ...
        e->build("", "EmbelProperties::itemChangeBorder");
        return;
    }
    ...
    if (source == "opacity") {
        setting->setValue(path, v.toInt());
        qDebug() << "EmbelProperties::itemChangeBorder" << index;
...
{
    if (G::isLogger) G::log("EmbelProperties::itemChangeText");
    QVariant v = idx.data(Qt::EditRole);
    QString source = idx.data(UR_Source).toString();
    QString parent = idx.parent().data(UR_Name).toString();
    ...
    /*
    qDebug() << "EmbelProperties::itemChangeText"
             << "source =" << source
             << "parent =" << parent;
//    */
    ...
    if (source == "metadataTemplate") {
        ...
        qDebug() << "EmbelProperties::itemChangeText" << "metadataTemplate" << value;
    ...
    if (source == "style") {
        qDebug() << "EmbelProperties::itemChangeText" << source;
        setting->setValue(path, v.toString());
...
//    qDebug() << "EmbelProperties::itemChangeGraphic" << "row =" << index << path << source << v.toInt();
    ...
    if (source == "size") {
        ...
        qDebug() << "EmbelProperties::itemChangeGraphic" << "g[index].size =" << g[index].size;
    }
    ...
    if (source == "style") {
        qDebug() << "EmbelProperties::itemChangeGraphic" << source;
        setting->setValue(path, v.toString());
...
*/
    if (G::isLogger) G::log("EmbelProperties:itemChangeShadowEffect:");
    QString path = "Embel/Templates/" + templateName + "/Styles/" + style + "/" + effectName + "/" + source;
//    qDebug() << "EmbelProperties::itemChangeShadowEffect" << path << source << v.toInt();
...
*/
    if (G::isLogger) G::log("EmbelProperties::itemChangeStrokeEffect");
    QString path = "Embel/Templates/" + templateName + "/Styles/" + style + "/" + effectName + "/" + source;
    //    qDebug() << "EmbelProperties::itemChangeStrokeEffect" << path << source << v.toInt();
...
*/
    if (G::isLogger) G::log("EmbelProperties::itemChangeGlowEffect");
    QString path = "Embel/Templates/" + templateName + "/Styles/" + style + "/" + effectName + "/" + source;
    //    qDebug() << "EmbelProperties::itemChangeGlowEffect" << path << source << v.toInt();
...
{
    if (G::isLogger) G::log("EmbelProperties::itemChangeBlurEffect");
    QString path = "Embel/Templates/" + templateName + "/Styles/" + style + "/" + effectName + "/" + source;
    qDebug() << "EmbelProperties::itemChangeBlurEffect" << path << source << v.toInt();
...
{
    if (G::isLogger) G::log("EmbelProperties::itemChangeSharpenEffect");
    QString path = "Embel/Templates/" + templateName + "/Styles/" + style + "/" + effectName + "/" + source;
...
{
    if (G::isLogger) G::log("EmbelProperties::itemChangeHighlighterEffect");
    QString path = "Embel/Templates/" + templateName + "/Styles/" + style + "/" + effectName + "/" + source;
//    qDebug() << "EmbelProperties::itemChangeHighlighterEffect" << path << source << v.toInt();
...
{
    if (G::isLogger) G::log("EmbelProperties::itemChangeBrightnessEffect");
    QString path = "Embel/Templates/" + templateName + "/Styles/" + style + "/" + effectName + "/" + source;
//    qDebug() << "EmbelProperties::itemChangeBrightnessEffect" << path << source << v.toInt();
...
{
    if (G::isLogger) G::log("EmbelProperties::itemChangeEmbossEffect");
    QString path = "Embel/Templates/" + templateName + "/Styles/" + style + "/" + effectName + "/" + source;
//    qDebug() << "EmbelProperties::itemChangeEmbossEffect" << path << source << v.toInt();
    // item 0
//    if (source == "contour") {
...
{
    if (G::isLogger) G::log("EmbelProperties::addTemplateHeader");
...
{
    if (G::isLogger) G::log("EmbelProperties::addTemplateItems");
    addExport();
    addStyles();
    addGeneral();
    addBorders();
...
{
    if (G::isLogger) G::log("EmbelProperties::addExport");
...
{
    if (G::isLogger) G::log("EmbelProperties::addGeneral");
...
{
    if (G::isLogger) G::log("EmbelProperties::addBorders");
    ...
    for (int row = 0; row < model->rowCount(bordersIdx); ++row) {
        ...
        qDebug() << "EmbelProperties::addBorders" << "DATA MODEL AND B VECTOR"
                 << "row =" << row
    ...
    qDebug() << "EmbelProperties::addBorders" << "BORDER VECTOR";
    for (int i = 0; i < b.length(); ++i) {
        qDebug().noquote()
            << i << " "
            << "name =" << b[i].name
...
{
    if (G::isLogger) G::log("EmbelProperties::addTexts");
    // Texts header (Root)
    i.name = "Texts";
    i.parentName = "???";
    i.isHeader = true;
...
{
    if (G::isLogger) G::log("EmbelProperties::addGraphics");
...
{
    if (G::isLogger) G::log("EmbelProperties::addStyles");
    //    qDebug() << "EmbelProperties::addStyles";
    // Styles header (Root)
    i.name = "Styles";
    i.parentName = "???";
    i.isHeader = true;
    ...
    int count = setting->childGroups().size();
    setting->endGroup();
//    qDebug() << "EmbelProperties::addStyles" << stylesList;
    styleMap.clear();
...
{
    if (G::isLogger) G::log("EmbelProperties::addStyle", name);
    // add the style header
    styleName = name;
    styleId = n;
//    qDebug() << "EmbelProperties::addStyle" << styleName << name << n;
...
{
    if (G::isLogger) G::log("EmbelProperties::addBorderHeaderButtons");
...
{
    if (G::isLogger) G::log("EmbelProperties::addEffectBtns");
    effectUpBtn = new BarBtn();
    effectUpBtn->setIcon(":/images/icon16/up.png", G::iconOpacity);
...
{
    if (G::isLogger) G::log("EmbelProperties::addBlurEffect", effectName);
    // styleName = parent
...
//    qDebug() << "EmbelProperties::addBlurEffect" << "effectName =" << effectName;
    if (effectName == "")
...
//    qDebug() << "EmbelProperties::addBlurEffect"
//             << "key =" << key
...
{
    if (G::isLogger) G::log("EmbelProperties::addSharpenEffect", effectName);
    // styleName = parent
    QString parentName = parIdx.data(UR_Name).toString();
    styleName = parentName;
    winnow_effects::Effect effect;
    effect.effectType = winnow_effects::sharpen;
//    qDebug() << "EmbelProperties::addSharpenEffect" << "effectName =" << effectName;
    if (effectName == "")
    ...
    // get settings data
    QString key = i.path + "/sortOrder";
//    qDebug() << "EmbelProperties::addSharpenEffect"
//             << "key =" << key
//             << "value =" << setting->value(key).toInt();
    if (setting->contains(key)) {
        i.sortOrder = setting->value(key).toInt();
...
{
    if (G::isLogger) G::log("EmbelProperties::addHighlighterEffect", effectName);
    // styleName = parent
    QString parentName = parIdx.data(UR_Name).toString();
    styleName = parentName;
    winnow_effects::Effect effect;
    effect.effectType = winnow_effects::highlighter;
//    qDebug() << "EmbelProperties::addHighlighterEffect" << "effectName =" << effectName;
    if (effectName == "")
...
{
    if (G::isLogger) G::log("EmbelProperties::addStrokeEffect", effectName);
    // styleName = parent
    QString parentName = parIdx.data(UR_Name).toString();
    styleName = parentName;
    winnow_effects::Effect effect;
    effect.effectType = winnow_effects::stroke;
//    qDebug() << "EmbelProperties::addStrokeEffect" << "effectName =" << effectName;
    if (effectName == "")
    ...
    // get settings data
    QString key = i.path + "/sortOrder";
//    qDebug() << "EmbelProperties::addStrokeEffect"
//             << "key =" << key
//             << "value =" << setting->value(key).toInt();
    if (setting->contains(key)) {
        i.sortOrder = setting->value(key).toInt();
//        qDebug() << "EmbelProperties::addStrokeEffect" << i.path << i.sortOrder;
    }
    else {
        i.sortOrder = model->rowCount(parIdx);
        setting->setValue(key, i.sortOrder);
...
{
    if (G::isLogger) G::log("EmbelProperties::addGlowEffect");
    // styleName = parent
    QString parentName = parIdx.data(UR_Name).toString();
    styleName = parentName;
    winnow_effects::Effect effect;
    effect.effectType = winnow_effects::glow;
//    qDebug() << "EmbelProperties::addGlowEffect" << "effectName =" << effectName;
    if (effectName == "")
    ...
    // get settings data
    QString key = i.path + "/sortOrder";
//    qDebug() << "EmbelProperties::addGlowEffect"
//             << "key =" << key
//             << "value =" << setting->value(key).toInt();
    if (setting->contains(key)) {
        i.sortOrder = setting->value(key).toInt();
//        qDebug() << "EmbelProperties::addGlowEffect" << i.path << i.sortOrder;
    }
    else {
        i.sortOrder = model->rowCount(parIdx);
        setting->setValue(key, i.sortOrder);
...
{
    if (G::isLogger) G::log("EmbelProperties::addBrightnessEffect");
    // styleName = parent
    QString parentName = parIdx.data(UR_Name).toString();
    styleName = parentName;
    winnow_effects::Effect effect;
...
{
    if (G::isLogger) G::log("EmbelProperties::addEmbossEffect");
    // styleName = parent
    QString parentName = parIdx.data(UR_Name).toString();
    styleName = parentName;
    winnow_effects::Effect effect;
...
{
    if (G::isLogger) G::log("EmbelProperties::addShadowEffect");
    // styleName = parent
    QString parentName = parIdx.data(UR_Name).toString();
    styleName = parentName;
    winnow_effects::Effect effect;
    effect.effectType = winnow_effects::shadow;
//    qDebug() << "EmbelProperties::addShadowEffect" << "effectName =" << effectName;
    if (effectName == "")
    ...
    // get settings data
    QString key = i.path + "/sortOrder";
//    qDebug() << "EmbelProperties::addShadowEffect"
//             << "key =" << key
//             << "value =" << setting->value(key).toInt();
    if (setting->contains(key)) {
        i.sortOrder = setting->value(key).toInt();
//        qDebug() << "EmbelProperties::addShadowEffect" << i.path << i.sortOrder;
    }
    else {
        i.sortOrder = model->rowCount(parIdx);
        setting->setValue(key, i.sortOrder);
...
{
    if (G::isLogger) G::log("EmbelProperties::newBorderFromBtn");
    newBorder("");
}

void EmbelProperties::newBorder(QString name)
{
    if (G::isLogger) G::log("EmbelProperties::newBorder");
...
//    selectionModel()->select(idx, QItemSelectionModel::Select | QItemSelectionModel::Rows);
    expand(bordersIdx);
    expand(idx);
    e->build("", "EmbelProperties::newBorder");
    updateBorderLists();
}

void EmbelProperties::newText()
{
    if (G::isLogger) G::log("EmbelProperties::newText");
    ...
    selectionModel()->clear();
    selectionModel()->select(idx, QItemSelectionModel::Select | QItemSelectionModel::Rows);
    e->build("", "EmbelProperties::newText");
}

void EmbelProperties::newGraphic()
{
    if (G::isLogger) G::log("EmbelProperties::newGraphic");
    ...
    selectionModel()->clear();
    selectionModel()->select(idx, QItemSelectionModel::Select | QItemSelectionModel::Rows);
    e->build("", "EmbelProperties::newGraphic");
}

void EmbelProperties::deleteTemplate()
{
    if (G::isLogger) G::log("EmbelProperties:deleteTemplate:");
    // a blank template name could delete all templates
    if (templateName == "") {
        qDebug() << "EmbelProperties::deleteTemplate" << "Error: Blank template name";
        return;
    }
    ...
    emit syncEmbellishMenu();
    syncWinnets("EmbelProperties::deleteTemplate");
}
...
*/
    if (G::isLogger) G::log("EmbelProperties::deleteItem");
    BarBtn *btn = qobject_cast<BarBtn*>(sender());
    QModelIndex idx = getItemIndex(btn->itemIndex);

        /*
    qDebug() << "EmbelProperties::deleteItem"
             << "btn->type =" << btn->type
...
//                */
    ...
    if (path == "Embel/Templates" || path == "Embel/Templates/" || path == "Embel/Templates//") {
        qDebug() << "EmbelProperties::deleteItem" << "ERROR: path to delte =" << path
...
//    */
    ...
    // refresh the graphicsScene
    e->build("", "EmbelProperties::deleteItem");
}

QString EmbelProperties::uniqueEffectName(QString styleName, int effectType, QString effectName)
{
    if (G::isLogger) G::log("EmbelProperties::uniqueEffectName");
//    using namespace winnow_effects;
    /*
    qDebug();
    qDebug() << "EmbelProperties::uniqueEffectName"
             << "styleName =" << styleName
...
//                */
    ...
    for (int i = 0; i < styleMap[styleName].length(); ++i) {
        ...
        qDebug() << "EmbelProperties::uniqueEffectName" << "Iterate styleMap[styleName]:"
                 << "i =" << i
                 << "effectType =" << effectType
    ...
    /*
    qDebug() << "EmbelProperties::uniqueEffectName" << "New effectName =" << effectName;
    qDebug();
//    */
    return effectName;
}
...
{
    if (G::isLogger) G::log("EmbelProperties::effectIndex");
    using namespace winnow_effects;
    // effects in style
//    qDebug() << "EmbelProperties::effectIndex" << style << effectName;
    for (int i = 0; i < styleMap[style].length(); ++i) {
//        qDebug() << "EmbelProperties::effectIndex"
//                 << "i =" << i
//                 << "effectName =" << effectName
...
{
    if (G::isLogger) G::log("EmbelProperties::flash");
...
//    if (G::isLogger) G::log("EmbelProperties::mouseMoveEvent");
       ...
       )
    {
        /*
        static int count = 0;
        qDebug() << "EmbelProperties::mouseMoveEvent" << count
                 << idx.data().toString()
...
//{
//    if (G::isLogger) G::log("EmbelProperties::mouseDoubleClickEvent");
//    QModelIndex idx = indexAt(event->pos());
...
*/
    if (G::isLogger) G::log("EmbelProperties::mousePressEvent");
...
{
    if (G::isLogger) G::log("EmbelProperties::treeChange");
    if (!idx.isValid()) return;
//    return;
    ...
    isStyle = true;
//    /*
    qDebug() << "EmbelProperties::treeChange"
...
//        qDebug() << "EmbelProperties::treeChange" << idx.data(UR_Name).toString();
...
{
    if (G::isLogger) G::log("EmbelProperties::okToSelect");
    QModelIndex parIdx = idx.parent();
...
//    qDebug() << "EmbelProperties::okToSelect" << "false for" << selName << effectList;
    return false;
}
...
{
    if (G::isLogger) G::log("EmbelProperties::diagnostics");
...
{
    if (G::isLogger) G::log("EmbelProperties::expandAllRows");
    expandAll();
}

void EmbelProperties::collapseAllRows()
{
    if (G::isLogger) G::log("EmbelProperties::collapseAllRows");
    collapseAll();
    expand(model->index(_templates,0));
}
...
{
    if (G::isLogger) G::log("EmbelProperties::solo");
...
{
    if (G::isLogger) G::log("EmbelProperties::expandRecursively");
...
{
    if (G::isLogger) G::log("EmbelProperties::test1");
//    syncWinnets();
//    e->test();
//    diagnosticVectors();
//    diagnosticStyles();
...
{
    if (G::isLogger) G::log("EmbelProperties::test2");
...
//        qDebug() << "EmbelProperties::addBorder" << i.value;
        setting->setValue(settingRootPath + i.key, i.value);
    }
    i.delegateType = DT_Color;
    i.type = "QString";
...
{
    if (G::isLogger) G::log("EmbelProperties::addText");
    // Name (used as node in settings and treeview)
...
{
    if (G::isLogger) G::log("EmbelProperties::addGraphic");
    // Name (used as node in settings and treeview)
    ...
    if (setting->contains(settingRootPath + i.key)) {
        i.value = setting->value(settingRootPath + i.key);
//        qDebug() << "EmbelProperties::addGraphic" << i.value;
