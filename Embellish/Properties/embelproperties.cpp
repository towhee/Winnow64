...
void EmbelProperties::newEffect()
{
    ...
    qDebug() << CLASSFUNCTION << "effect =" << effect;
...
void EmbelProperties::renameSettingKey(QString path, QString oldName, QString newName)
{
    ...
    if (oldName == "") {
        qDebug() << CLASSFUNCTION << "Error: Blank name";
        return;
    }
...
*/
    if (G::isLogger) G::log(CLASSFUNCTION, name);
    if (name == "") return;
    templateName = name;
...
//    qDebug() << CLASSFUNCTION << templateId << templateName << templatePath;
}
...
QString EmbelProperties::uniqueTileName(QString name)
{
    ...
    setting->endGroup();
    qDebug() << CLASSFUNCTION << name << keys;
    bool nameExists = true;
    QString newName = name;
    int count = 0;
    while (nameExists) {
...
void EmbelProperties::copyStyle()
{
    ...
    qDebug() << CLASSFUNCTION << toTemplate << copyName;
...
//    qDebug() << CLASSFUNCTION << keys; return true;
    /*
    setting->beginGroup("InfoTemplates");
...
//    QString tPath = dstPath + "/" + srcTemplateName + ".template";
    ...
    if (!tFile.open(QIODevice::ReadOnly)) {
        qDebug() << CLASSFUNCTION << "Could not open template file.";
...
void EmbelProperties::manageGraphics()
{
    ...
    dlg.exec();
    updateGraphicList();
    qDebug() << CLASSFUNCTION << graphicList;
}
...
void EmbelProperties::newTemplate()
{
    ...
    templateName = Utilities::inputText("New Template",
                                        ...
                                        templateList);
    qDebug() << CLASSFUNCTION << templateName;
...
void EmbelProperties::newTemplateFromImport(QString name)
{
    ...
    qDebug() << CLASSFUNCTION << "templateListEditor->setValue(templateName)" << name;
    templateListEditor->setValue(name);
    emit syncEmbellishMenu();
    syncWinnets();
}
...
//                */
    ...
    for (int i = 0; i < model->rowCount(par); ++i) {
        ...
        qDebug() << CLASSFUNCTION << "Before swap"
                 << "row =" << i
...
//    /*
    for (int i = 0; i < model->rowCount(par); ++i) {
        QModelIndex idx = model->index(i, 0, par);
        QString name = idx.data().toString();
        qDebug() << CLASSFUNCTION << "After  swap"
                 << "row =" << i
                 << "name =" << name.leftJustified(8)
                 << "order =" << idx.data(UR_SortOrder).toInt()
                    ;
...
//    QModelIndex idx = btn->index;
    if (!idx.isValid()) {
        qDebug() << CLASSFUNCTION << "idx.isValid() =" << idx.isValid();
        return;
    }
...
{
    if (G::isLogger) G::log(CLASSFUNCTION, "Style: " + style);
    if (!styleMap.contains(style)) {
        qDebug() << CLASSFUNCTION << style << "not in list";
        return false;
    }
...
//    qDebug() << CLASSFUNCTION << "loadImage..."
//             << "mw3->dm->currentFilePath =" << mw3->dm->currentFilePath;
    mw3->imageView->loadImage(mw3->dm->currentFilePath, CLASSFUNCTION);
...
void EmbelProperties::itemChangeExport(QModelIndex idx, QVariant v, QString source)
{
    ...
    if (source == "saveMethod") {
        ...
        qDebug() << CLASSFUNCTION << source << saveMethod;
        int row = idx.row();
...
//    */
    ...
    if (source == "metadataTemplate") {
        ...
        qDebug() << CLASSFUNCTION << "metadataTemplate" << value;
...
//    qDebug() << CLASSFUNCTION << "row =" << index << path << source << v.toInt();
    ...
    if (source == "size") {
        ...
        qDebug() << CLASSFUNCTION << "g[index].size =" << g[index].size;
    }
...
//    qDebug() << CLASSFUNCTION << path << source << v.toInt();
...
void EmbelProperties::addBorders()
{
    ...
    for (int row = 0; row < model->rowCount(bordersIdx); ++row) {
        ...
        qDebug() << CLASSFUNCTION << "DATA MODEL AND B VECTOR"
                 << "row =" << row
    ...
    qDebug() << CLASSFUNCTION << "BORDER VECTOR";
    for (int i = 0; i < b.length(); ++i) {
        qDebug().noquote()
            << i << " "
            << "name =" << b[i].name
...
void EmbelProperties::addStyles()
{
    ...
    int count = setting->childGroups().size();
    setting->endGroup();
//    qDebug() << CLASSFUNCTION << stylesList;
    styleMap.clear();
...
void EmbelProperties::addStyle(QString name, int n)
{
    ...
    styleName = name;
    styleId = n;
//    qDebug() << CLASSFUNCTION << styleName << name << n;
...
{
    if (G::isLogger) G::log(CLASSFUNCTION, effectName);
    // styleName = parent
...
//             << "value =" << setting->value(key).toInt();
    if (setting->contains(key)) {
        i.sortOrder = setting->value(key).toInt();
//        qDebug() << CLASSFUNCTION << i.path << i.sortOrder;
    }
    else {
        i.sortOrder = model->rowCount(parIdx);
        setting->setValue(key, i.sortOrder);
...
void EmbelProperties::deleteTemplate()
{
    ...
    if (templateName == "") {
        qDebug() << CLASSFUNCTION << "Error: Blank template name";
        return;
    }
...
//                */
    ...
    if (path == "Embel/Templates" || path == "Embel/Templates/" || path == "Embel/Templates//") {
        qDebug() << CLASSFUNCTION << "ERROR: path to delte =" << path
...
//                */
    ...
    for (int i = 0; i < styleMap[styleName].length(); ++i) {
        ...
        qDebug() << CLASSFUNCTION << "Iterate styleMap[styleName]:"
                 << "i =" << i
                 << "effectType =" << effectType
    ...
    /*
    qDebug() << CLASSFUNCTION << "New effectName =" << effectName;
    qDebug();
//    */
    return effectName;
}
...
int EmbelProperties::effectIndex(QString style, QString effectName)
{
    ...
    using namespace winnow_effects;
    // effects in style
//    qDebug() << CLASSFUNCTION << style << effectName;
...
//        qDebug() << CLASSFUNCTION << idx.data(UR_Name).toString();
...
//    qDebug() << CLASSFUNCTION << "false for" << selName << effectList;
    return false;
}
...
//    qDebug() << CLASSFUNCTION << borderName << count;
