...
bool Jpeg2::getWidthHeight(MetadataParameters &p, int &w, int &h)
{
...
bool Jpeg2::getDimensions(MetadataParameters &p, ImageMetadata &m)
{
    bool isBigEnd = true;                  // only IFD/EXIF can be little endian
    quint32 marker = 0;
    p.offset += 2;
...
bool Jpeg2::getDimensions2(MetadataParameters &p, ImageMetadata &m)
{
...
bool Jpeg2::parse(MetadataParameters &p,
                 ImageMetadata &m,
                 IFD *ifd,
                 IPTC *iptc,
                 Exif *exif,
                 GPS *gps)
{
    ...
    while (!foundEndian) {
        ...
        if (order == 0x4949 || order == 0x4D4D) {
            ...
            foundEndian = true;
            /*
            qDebug() << "Jpeg2::parse" << p.file.fileName()
                     << "isBigEnd =" << isBigEnd
...
void Jpeg2::getJpgSegments(MetadataParameters &p, ImageMetadata &m)
{
    /*
...
void Jpeg2::readAppSegments(MetadataParameters &p)
{
    int len;
...
void Jpeg2::parseFrameHeader(MetadataParameters &p, uint marker, quint16 len)
{
...
void Jpeg2::parseHuffmanTable(MetadataParameters &p, quint16 len)
{
    bool isDebug = false;
...
void Jpeg2::parseQuantizationTable(MetadataParameters &p, quint16 len)
{
    /*
...
void Jpeg2::parseSOSHeader(MetadataParameters &p, quint16 len)
{
...
int Jpeg2::huff2Signed(uint val, uint bits)
{
...
void Jpeg2::decodeScan(MetadataParameters &p)
{
    p.file.seek(0);
...
void Jpeg2::decodeScan(QString fPath, QImage &image)
{

}

void Jpeg2::decodeScan(QFile &file, QImage &image)
{
    // qDebug() << "Jpeg2::decodeScan" << file.fileName();
    ...
    if (p.file.open(QIODevice::ReadOnly)) {
        // qDebug() << "Jpeg2::decodeScan" << "Open " << p.file.fileName();
        bool isBigEnd = true;
        if (u.get16(p.file.read(2), isBigEnd) != 0xFFD8) {
            QString msg = "JPG does not start with 0xFFD8.";
...
void Jpeg2::decodeScan(QByteArray &ba, QImage &image)
{
...
*/
    ...
    t.start();
    buildMask();
    qDebug() << "buildMask             µs =" << t.nsecsElapsed() /1000; t.restart();
    buildIdctLookup();
    qDebug() << "buildIdctLookup       µs =" << t.nsecsElapsed() /1000; t.restart();
    ...
    for (int mcuRow = 0; mcuRow < mcuRows; ++mcuRow) {
        dcDiff[0] = 0; dcDiff[1] = 0; dcDiff[2] = 0; dcDiff[3] = 0;
        for (int mcuCol = 0; mcuCol < mcuCols; ++mcuCol) {
            processMCU(ba, offset, buf, consumed, bitsToResetMarker, eos);
            qDebug() << "processMCU            µs =" << t.nsecsElapsed() /1000; t.restart();
            // IDCT transform and level shift
            idctTransformMCU();
            qDebug() << "idctTransformMCU      µs =" << t.nsecsElapsed() /1000; t.restart();
            // RGB transform
            rgbTransformMCU();
            qDebug() << "rgbTransformMCU       µs =" << t.nsecsElapsed() /1000; t.restart();
            // add mcu rgb to QImage image using memcpy scanline
            appendMCU2QImage();
            qDebug() << "appendMCU2QImage      µs =" << t.nsecsElapsed() /1000; t.restart();
        } // end row of MCUs

        // add scanLines for this row of mcus
        appendMCUScanlines2QImage(im, mcuRow);
        qDebug() << "appendMCUScanlines2QImage µs =" << t.nsecsElapsed() /1000; t.restart();
...
QImage Jpeg2::turboDecode(QString &filePath)
{
    /*
...
*/
    ...
    if (!tjInstance) {
        qDebug() << "Jpeg2::turboDecode(fPath)" << "Failed to initialize TurboJPEG: " << tjGetErrorStr();
        return QImage();
    }
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        qDebug() << "Jpeg2::turboDecode(fPath)" << "Failed to open JPEG file.";
    ...
    if (tjDecompressHeader3(tjInstance, (unsigned char*)ba.data(), ba.size(),
        ...
        qDebug() << "Jpeg2::turboDecode(fPath)" << "Failed to read JPEG header: " << tjGetErrorStr();
        tjDestroy(tjInstance);
        return QImage();
    }
    ...
    if (!imgBuffer) {
        qDebug() << "Jpeg2::turboDecode(fPath)" << "Failed to allocate memory for decompressed image.";
        tjDestroy(tjInstance);
        return QImage();
    }
...
QImage Jpeg2::turboDecode(QByteArray &ba)
{
    /*
    libjpeg-turbo library
*/
...
void Jpeg2::bufAppend(uint &buf, quint8 byte, uint &consumed)
{
    if (consumed > 7) {
...
uint Jpeg2::bufExtract(uint &buf, uint nBits, uint &consumed)
{
...
uint Jpeg2::bufPeek(uint &buf, uint nBits)
{
    return (buf & mask[nBits]) >> (32 - nBits);
}

void Jpeg2::buildMask()
{
    for (uint i = 0; i != 32; ++i) {
        uint iMask = (1 << i) - 1;
        iMask <<= 32 - i;
...
// void Jpeg2::processMCU(QByteArray &ba, quint32 &offset, uint &buf,
                       ...
                       uint &consumed, int &bitsToResetMarker, bool &eos)
{
    ...
    // Initialize MCU to zero
    std::array<std::array<std::array<int, MCU_SIZE>, MCU_SIZE>, NUM_COMPONENTS> mcu = {};
            ...
            while (consumed > 7 && !eos) {
                if (!eos) {
                    ...
                    if (nextByte == 0xFF) {
                        ...
                        if (markerByte != 0) {
                            bitsToResetMarker = 32 - consumed;
                            if (markerByte == 0xD9) eos = true;
                        }
                    }
                    if (!eos && nextByte != 0xFF) {
                        bufAppend(buf, nextByte, consumed);
                    }
                }
            }

            int hTbl = (c == 0) ? ((m == 0) ? 0x00 : 0x10) : ((m == 0) ? 0x01 : 0x11);

            bool endOfBlock = false;
            for (uint huffLength = 1; huffLength < 17; ++huffLength) {
                auto &huffMap = dhtMap[hTbl][huffLength];
                if (!huffMap.empty()) {
                    uint huffCode = bufPeek(buf, huffLength);
                    if (huffMap.count(huffCode)) {
                        uint huffVal = huffMap[huffCode] & 0xF;
                        int huffRepeat = huffMap[huffCode] >> 4;
                        uint huffResult = bufExtract(buf, huffVal, consumed);
                        int huffSignedResult = huff2Signed(huffResult, huffVal);
                        ...
                        if (bitsToResetMarker) {
                            bitsToResetMarker -= huffLength + huffVal;
                            if (bitsToResetMarker < 8) {
                                uint markerBuf = buf >> (32 - bitsToResetMarker);
                                if ((markerBuf + 1) == (1 << bitsToResetMarker)) {
                                    buf <<= bitsToResetMarker;
                                    consumed += bitsToResetMarker;
                                    bitsToResetMarker = 0;
                                }
                            }
                        ...
                        if ((huffMap[huffCode] & 0xF0) == 0 && m > 0) {
                            endOfBlock = true;
                            for (int i = m + 1; i < 64; ++i) {
                                int y = zz[i][0];
                                int x = zz[i][1];
...
void Jpeg2::appendMCUScanlines2QImage(QImage &im, int &mcuRow)
{
    for (int y = 0; y < 8; y++) {
        QByteArray data;
...
void Jpeg2::rptMCU(int col, int row)
{
    qDebug();
...
void Jpeg2::rptIDCT(int col, int row)
{
    qDebug();
    QString mcuCoord = "[" + QString::number(col) + "," + QString::number(row) + "]";
...
void Jpeg2::rptRGB(int col, int row)
{
    qDebug();
    QString mcuCoord = "[" + QString::number(col) + "," + QString::number(row) + "]";
