...
Heic::Heic(/*QFile &file*/)
{
    ...
    QFileInfo info(file);
    qDebug();
    qDebug() << "Heic::Heic" << info.filePath() << "\n";
...
bool Heic::getHeifBox(QString &type, quint32 &offset, quint32 &length)
{
    ...
    G::error("Heic::getHeifBox", fPath, err);
    qDebug();
...
bool Heic::ftypBox(quint32 &offset, quint32 &length)
{
    ...
    if (!isHeic) {
        ...
        G::error("Heic::ftypBox", fPath, err);
        qDebug() << "Heic::ftypBox" << "heic not found";
        return false;
    }
    offset += length;
    return true;
...
bool Heic::ilocBox(quint32 &offset, quint32 &length)
{
    ...
    if (ilocItemCount > 100) {
        qDebug() << "Heic::ilocBox" << "*** Quiting because ilocItemCount =" << ilocItemCount;
        offset += length;
        return false;
    }
    for (int i = 0; i < ilocItemCount; i++) {
        ...
        if (extent_count > 100) {
            ...
            qDebug() << "Heic::ilocBox" << "*** Quiting because extent_count =" << extent_count;
            offset += length;
            return false;
        }
...
bool Heic::iinfBox(quint32 &offset, quint32 &length)
{
    ...
    if (entry_count == 0) {
        // err
        qDebug() << "Heic::iinfBox" << "No iint entries found";
        offset += length;
        return false;
    }
    ...
    for (int i = 0; i < entry_count; i++) {
        ...
        qDebug() << "Heic::iinfBox" << "   Item Info Entry " << i;
...
bool Heic::iprpBox(quint32 &offset, quint32 &length)
{
    ...
    if (ipcoType != "ipco") {
        ...
        G::error("Heic::iprpBox", fPath, err);
