...
Heic::Heic(/*QFile &file*/)
{
    ...
    QFileInfo info(file);
    qDebug();
    qDebug() << CLASSFUNCTION << info.filePath() << "\n";
...
bool Heic::parseLibHeif(MetadataParameters &p, ImageMetadata &m, IFD *ifd, Exif *exif, GPS *gps)
{
    ...
    if (error.code) {
        qDebug() << CLASSFUNCTION << "heif_context_read_from_file" << error.message;
        return false;
    }
    /*
    qDebug() << CLASSFUNCTION << "Error:"
             << "code =" << error.code
             << "subcode =" << error.subcode
             << "message =" << error.message
            ;
//    */
//    qDebug() << CLASSFUNCTION << "Read heif context =" << t.nsecsElapsed() << fPath;

    // get a handle to the primary image
    heif_image_handle* handle = nullptr;
    error = heif_context_get_primary_image_handle(ctx, &handle);
    if (error.code) {
        qDebug() << CLASSFUNCTION << "heif_context_get_primary_image_handle" << error.message;
        return false;
    }
    ...
    if (error.code) {
        qDebug() << CLASSFUNCTION << "heif_image_handle_get_metadata" << error.message;
        return false;
    }
...
//    Utilities::saveByteArrayAsFile("D:/Pictures/_HEIC/test.dat", exifData);
    ...
    while (!foundEndian) {
        ...
        if (count > 100) {
            ...
            G::error(CLASSFUNCTION, fPath, "Endian order not found.");
            p.buf.close();
            return false;
        }
    }
...
//    // read IFD1
//    qDebug() << CLASSFUNCTION << "nextIFDOffset IFD1 =" << nextIFDOffset;
//    if (nextIFDOffset) {
//        p.hdr = "IFD1";
...
bool Heic::decodePrimaryImage(QString &fPath, QImage &image)
{
    ...
    error = heif_decode_image(handle,
                      ...
                      nullptr);
    if (error.code || !img) {
        qDebug() << CLASSFUNCTION << "heif_decode_image" << error.message;
        return false;
    }
...
bool Heic::ftypBox(quint32 &offset, quint32 &length)
{
    if (length == 0) {
        // err
        qDebug() << CLASSFUNCTION << "ftyp not found";
        return false;
    }
    ...
    if (!isHeic) {
        ...
        G::error(CLASSFUNCTION, fPath, err);
        qDebug() << CLASSFUNCTION << "heic not found";
        return false;
    }
    offset += length;
    return true;
...
bool Heic::drefBox(quint32 &offset, quint32 &length)
{
    ...
    qDebug();
    qDebug() << CLASSFUNCTION << "entry_count =" << entry_count;

    for (quint32 i = 0; i < entry_count; i++) {
        if (offset > drefEnd) return false;
        nextHeifBox(length, type);
        qDebug() << CLASSFUNCTION << "entry#" << i;
...
bool Heic::colrBox(quint32 &offset, quint32 &length)
{
    ...
    qDebug() << CLASSFUNCTION << "colrType =" << colrType;
    ...
    if (colrType == "nclx") {
        ...
        qDebug() << CLASSFUNCTION << colorProfile;
    }
    else {
        // err
        qDebug() << CLASSFUNCTION << "*** color type" << colrType << "is not recognized";
        offset += length;
        return false;
    }
...
bool Heic::pixiBox(quint32 &offset, quint32 &length)
{
    ...
    for (quint8 i = 0; i < num_channels; i++) {
        ...
        qDebug() << CLASSFUNCTION << "channel#" << i << "bits_per_channel =" << bits_per_channel;
    }
...
bool Heic::irotBox(quint32 &offset, quint32 &length)
{
    ...
    qDebug() << CLASSFUNCTION << "angle =" << angle << "angle degrees =" << angle_degrees;
...
bool Heic::ilocBox(quint32 &offset, quint32 &length)
{
    ...
    if (ilocItemCount > 100) {
        qDebug() << CLASSFUNCTION << "*** Quiting because ilocItemCount =" << ilocItemCount;
        offset += length;
        return false;
    }
    for (int i = 0; i < ilocItemCount; i++) {
        ...
        if (extent_count > 100) {
            ...
            G::error(CLASSFUNCTION, fPath, err);
            qDebug() << CLASSFUNCTION << "*** Quiting because extent_count =" << extent_count;
            offset += length;
            return false;
        }
        for (int j = 0; j < extent_count; j++) {
            ...
            qDebug() <<CLASSFUNCTION << "    Extent:" << i
                     << "extent_offset" << extent_offset
                     << "extent_length" << extent_length;
        }
    }
    qDebug() << CLASSFUNCTION << "file.pos()" << file.pos();
    //    if (hdlrType != "pict") {
    //        // err
    //        return false;
    //    }
...
bool Heic::iinfBox(quint32 &offset, quint32 &length)
{
    ...
    qDebug() << CLASSFUNCTION << "iint entry count =" << entry_count << file.pos();
    if (entry_count == 0) {
        // err
        qDebug() << CLASSFUNCTION << "No iint entries found";
        offset += length;
        return false;
    }
    ...
    for (int i = 0; i < entry_count; i++) {
        ...
        qDebug() << CLASSFUNCTION << "   Item Info Entry " << i;
...
bool Heic::sitrBox(quint32 &offset, quint32 &length)
{
    ...
    for (int i = 0; i < reference_count; i++) {
        ...
        qDebug() << CLASSFUNCTION << i << ": to_item_ID =" << to_item_ID;
    }
    offset += length;
    return true;
}
...
bool Heic::irefBox(quint32 &offset, quint32 &length)
{
    ...
    qDebug() << CLASSFUNCTION << "offset =" << offset
...
//        else sitrBoxL(sitrOffset, sitrLength);
        qDebug() << CLASSFUNCTION << "irefEndOffset =" << irefEndOffset << "sitrOffset" << sitrOffset;
    }
...
{
    qDebug() << CLASSFUNCTION << "file position:" << file.pos();
    ...
    qDebug() << CLASSFUNCTION << "configurationVersion =" << configurationVersion;
    qDebug() << CLASSFUNCTION << "general_profile_space =" << general_profile_space;
    qDebug() << CLASSFUNCTION << "general_tier_flag =" << general_tier_flag;
    qDebug() << CLASSFUNCTION << "general_profile_idc =" << general_profile_idc;
    qDebug() << CLASSFUNCTION << "general_profile_compatibility_flags =" << general_profile_compatibility_flags;
    qDebug() << CLASSFUNCTION << "general_constraint_indicator_flags =" << general_constraint_indicator_flags;
    qDebug() << CLASSFUNCTION << "general_level_idc =" << general_level_idc;
    qDebug() << CLASSFUNCTION << "min_spatial_segmentation_idc =" << min_spatial_segmentation_idc;
    qDebug() << CLASSFUNCTION << "parallelismType =" << parallelismType;
    qDebug() << CLASSFUNCTION << "chroma_format_idc =" << chroma_format_idc;
    qDebug() << CLASSFUNCTION << "bit_depth_luma_minus8 =" << bit_depth_luma_minus8;
    qDebug() << CLASSFUNCTION << "bit_depth_chroma_minus8 =" << bit_depth_chroma_minus8;
    qDebug() << CLASSFUNCTION << "avgFrameRate =" << avgFrameRate;
    qDebug() << CLASSFUNCTION << "constantFrameRate =" << constantFrameRate;
    qDebug() << CLASSFUNCTION << "numTemporalLayers =" << numTemporalLayers;
    qDebug() << CLASSFUNCTION << "temporalIdNested =" << temporalIdNested;
    qDebug() << CLASSFUNCTION << "lengthSizeMinusOne =" << lengthSizeMinusOne;
    qDebug() << CLASSFUNCTION << "numOfArrays =" << numOfArrays;
    ...
    for (int i = 0; i < numOfArrays; i++) {
        ...
        qDebug() << CLASSFUNCTION << "    array # =" << i;
        qDebug() << CLASSFUNCTION << "    array_completeness =" << array_completeness;
        qDebug() << CLASSFUNCTION << "    NAL_unit_type =" << NAL_unit_type;
        qDebug() << CLASSFUNCTION << "    numNalus =" << numNalus;

        for (quint16 j = 0; j < numNalus; j++) {
            quint16 nalUnitLength = Utilities::get16(file.read(2));
            QByteArray nalUnit = file.read(nalUnitLength);
            qDebug() << CLASSFUNCTION << "        nalUnitLength =" << nalUnitLength;
        }
    }
...
bool Heic::ispeBox(quint32 &offset, quint32 &length)
{
    ...
    qDebug() << CLASSFUNCTION << "image_width =" << image_width;
    qDebug() << CLASSFUNCTION << "image_height =" << image_height << "\n";
...
bool Heic::iprpBox(quint32 &offset, quint32 &length)
{
    ...
    if (ipcoType != "ipco") {
        ...
        G::error(CLASSFUNCTION, fPath, err);
        qDebug() << CLASSFUNCTION << "ipco not found in iprp box";
        return false;
    }
