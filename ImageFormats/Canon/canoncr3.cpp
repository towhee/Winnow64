...
bool CanonCR3::getHeifBox(QString &type, quint32 &offset, quint32 &length)
{
   ...
   qDebug() << "CanonCR3::getHeifBox"
            ...
            << "(Undefined)"
               ;
    // if (G::isErrorLogger) G::error("Box type " + type + " unknown.", "CanonCR3::getHeifBox", m->fPath);
    offset += length;
    return true;
//    return false;
}
...
bool CanonCR3::ftypBox(quint32 &offset, quint32 &length)
{
    ...
    if (length == 0) {
        // err
        // if (G::isErrorLogger) G::error("ftyp not found.", "CanonCR3::ftypBox", m->fPath);
    ...
    if (!isCR3) {
        // err
        // if (G::isErrorLogger) G::error("crx not found", "CanonCR3::ftypBox", m->fPath);
...
bool CanonCR3::colrBox(quint32 &offset, quint32 &length)
{
    ...
    if (colrType == "nclx") {
        ...
        // err
        if (G::isErrorLogger) // G::error("Color type " + colrType + " is not recognized", "CanonCR3::colrBox", m->fPath);
        offset += length;
        return false;
    }
...
bool CanonCR3::ilocBox(quint32 &offset, quint32 &length)
{
    ...
    for (int i = 0; i < ilocItemCount; i++) {
        ...
        if (extent_count > 100) {
            ...
            // if (G::isErrorLogger) G::error(err, "CanonCR3::ilocBox", m->fPath);
...
//    qDebug() << "CanonCR3::iinfBox" << "iint entry count =" << entry_count << p->file.pos();
    if (entry_count == 0) {
        ...
        // if (G::isErrorLogger) G::error(err, "CanonCR3::iinfBox", m->fPath);
        offset += length;
        return false;
    }
...
bool CanonCR3::iprpBox(quint32 &offset, quint32 &length)
{
    ...
    if (ipcoType != "ipco") {
        ...
        // if (G::isErrorLogger) G::error(err, "CanonCR3::iprpBox", m->fPath);
